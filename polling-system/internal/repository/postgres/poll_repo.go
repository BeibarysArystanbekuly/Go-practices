...
func (r *PollRepo) Create(p *poll.Poll, options []poll.Option) (int64, error) {
    tx, err := r.db.Begin()
    if err != nil {
        return 0, err
    }
    ...
    for i := range options {
        ...
        if err := tx.QueryRow(queryOpt, options[i].PollID, options[i].Text).
...
func (r *PollRepo) GetByID(id int64) (*poll.Poll, []poll.Option, error) {
    p := &poll.Poll{}
    err := r.db.QueryRow(`
...
func (r *PollRepo) List(status *string) ([]poll.Poll, error) {
    query := `
        SELECT id, title, description, status, starts_at, ends_at, creator_id, created_at, updated_at
        FROM polls
    `
...
func (r *PollRepo) UpdateStatus(id int64, status string) error {
    _, err := r.db.Exec(`UPDATE polls SET status = $1, updated_at = now() WHERE id = $2`, status, id)
    return err
}
